...
<head>
    ...
    <link rel="stylesheet" href="css/main.css?id=<?php echo rand(0,10000000);?>">
...
<body>
    ...
    function upgradePlayer() {
        ...
        if (levels.length > myPlayerIdIndex && jsonMain != null) {
            ...
            new WindowConfirmUpgrade(id_player, "<?php echo $locale["TEXT_LEVEL_UP"];?> " + (myPlayerIdIndex + 1),
                "<?php echo $locale["TEXT_MINIMAL_RANG"];?> " + rangArray[
                    jsonMain.min_rang] + ".<br/><?php echo $locale["PRICE_FOR_IT"];?> " + (jsonMain.price) +
    ...
    class WindowConfirmUpgrade {
        constructor(id_level, name, text, func, json, isCan) {
            ...
            if (json != null) {
                ...
                if (json.health != null) {
                    element +=
                        '<div class="base_element_information"><div class="base_element_information_left"><img src="img/heart.png" width="12px"> <?php echo $locale["TEXT_HEALTH"];?>: </div><div  class="base_element_information_right">+' +
                        json.health + '</div></div>'
                }
    ...
    class LoaderView {
                ...
                this.interval = setInterval(function() {
                    ...
                    document.getElementById("loader_view_loader").innerHTML =
                        "<?php echo $locale["TEXT_LOADING"];?> " + loader.array[
                            loader.i] + " " + loader.getCountOfLoading() + "%";
        ...
        getCountOfLoading() {
            ...
            return Math.floor((loader.loader.length - count) * 100 / loader.loader.length);
        }
        stop() {
    ...
    function add_styles() {
        ...
        for (var i = 0; i < jsonPlayerFromBase.breath[0].styles.length; i++) {
            ...
            player.key.push(i + 1);
            if (isMobile() == false) {
                var html = '<div class="element_of_style"  ontouchstart="setStyle(' + i + ')"  onClick="setStyle(' +
                    i +
    ...
    class BallModel {
        constructor(json) {
            ...
            this.x = json.x;
            this.y = json.y;
            this.animate = images.get("animate_fireballs_0").array;
            this.index = randomIntFromInterval(0, animate.length - 1);
            this.draw = function(data) {
                if (this.animate != null) {

                    var ppx = this.x - player.position.x + example.width / 3;
                    var ppy = this.y - player.position.y + example.height / 3 *
                        hightTop;
                    //  console.log(co);
                    data.drawImage(this.animate[this.index], ppx, ppy);

                }
            }
            this.update = function(json) {

                this.x = json.x / START_SIZE * SIZE;

                this.y = json.y / START_SIZE * SIZE;
        ...
        constructor(json) {
            ...
            this.animation = setInterval(function() {
                ...
                if (player.id != null) {
                    ...
                    for (var [key, value] of player.balls) {
                        var el = value;
                        el.index++;
                        if (el.index >= el.animate.length) {
                            el.index = 0;
            ...
            this.draw = function(data) {
                ...
                if (player != null && player.animate != null) {
                    ...
                    for (var i = -0; i < player.lines.length; i++) {
                        ...
                        drawLine(data, [line.x / START_SIZE * SIZE - this.player.position.x + this.canvas
                            .width / 3, line.y / START_SIZE * SIZE - this.player.position.y + this
                            .canvas.height / 3 * hightTop
                        ], [line.endx / START_SIZE * SIZE - this.player.position.x + this.canvas.width /
                            3, line.endy / START_SIZE * SIZE - this.player.position.y + this.canvas
                            .height / 3 * hightTop
                        ], 'white', line.lineWidth);
                    }
                    for (var [key, value] of player.balls) {
                        value.draw(data);
    ...
    class Player {
        ...
        downKey(key) {
            ...
            setStyle(this.key[key] - 1);
            this.downKeyInt = key;
            document.getElementById("styleInt").innerHTML = this.key[key];
        }
        downKeyRight() {
            this.downKeyInt++;
            if (this.downKeyInt >= this.key.length) {
                this.downKeyInt = 0;
            }
        ...
        downKeyLeft() {
            ...
            if (this.downKeyInt < 0) {
                this.downKeyInt = this.key.length - 1;
            }
            this.downKey(this.downKeyInt);
        }
        getPlayerIs() {
    ...
    */
    ...
    var on = false;
    var scale = isMobile() == true ? 2.8 : 1.35;
    ...
    var GRAVITY = 12;
    var indexMoving = 0;
    var powerInventory = 1;
    var nitroInventory = 1;
    var amorInventory = 1;
    ...
    function toggleFullScreen() {
        ...
        if (document.fullscreen != null && document.fullscreen == false) {
    ...
    async function loadJsonSprites() {
        var json = await loadJSON("res/animate.json?id=" + Math.random());
        json = JSON.parse(json)
    ...
    async function loadJsonFromPlayer(playerPreview) {
        ...
        var json = await loadJSON("img/union/" + playerPreview.id + "/index.json?id=" + Math.random());
        json = JSON.parse(json);
        ...
        var index = 0;
        await imageLoader.loadImage("img/union/" + playerPreview.id + "/stay_small_web.png?id=" + v, playerPreview
            .id +
            "_stay");

        for (; i < end; i++) {

            await imageLoader.loadImage("img/union/" + playerPreview.id + "/" + (json.run.folder + "_small_web") +
        ...
        for (; i < ends; i++) {
            ...
            var index = 0;
            for (; j < end; j++) {
                await imageLoader.loadImage("img/union/" + playerPreview.id + "/" + (json1.folder + "_small_web") +
    ...
    window.onload = function() {
        ...
        connection = new WebSocket('ws://192.168.42.41:9999');
        connection.onopen = async function(e) {
            console.log('Connected! ' + e);
            await run()

            //auth(null)
            // connection.send("auth;login;false;");w
        };
        ...
        connection.onclose = function(a) {
            document.getElementById("errorWindow").style = "position: fixed;";
        ...
        connection.onmessage = function(e) {
            ...
            if (b[0] == "reg") {
                ...
                if (b[1] == "exist") {
                    ...
                    document.getElementById("reguser").readOnly = false;
                    document.getElementById("regpass").readOnly = false;

                    alert("<?php echo $locale["USER_ALREADY_EXIST"];?>")

                }
                if (b[1] == "password_unvalidate") {
                    $("#buttonReg").show(300);
                    ...
                    alert("<?php echo $locale["INCORRECT_PASSWORD_OR_LOGIN"];?>")
                }
                if (b[1] == "login_unvalidate") {
                    $("#buttonReg").show(300);
            ...
            if (b[0] == "battle") {
                ...
                if (b[1] == "npcs") {
                    var jsonMain = JSON.parse(e.data.replace("battle;npcs;", ""));
                    var jsonArray = jsonMain.npcs
                    for (var i = 0; i < jsonArray.length; i++) {
                        ...
                        if (json.balls != null) {
                            var idNpc = b[2]
                            var npc = element
                            var balls = json.balls;
                            for (var i = 0; i < balls.length; i++) {
                                var element = balls[i];
                                var line = npc.balls.get(element.id);
                                if (line == null) {
                                    npc.balls.set(element.id, new BallModel(element));
                                } else {
                                    line.update(element);
                                }
                            }
                ...
                if (b[1] == "balls_remove") {
                    var idNpc = b[2]
                    var npc = npcMap.get(idNpc);
                    if (npc != null) {

                        // console.log(b[3])
                        npc.balls.delete(b[3]);
                ...
                if (b[1] == "balls") {
                    ...
                    var npc = npcMap.get(idNpc);
                    if (npc != null) {
                        var json = JSON.parse(e.data.replace("battle;balls;" + idNpc + ";", ""))
                        var balls = json.balls;
                        for (var i = 0; i < balls.length; i++) {
                            var element = balls[i];
                            var line = npc.balls.get(element.id);
                ...
                if (b[1] == "map_init") {
                    ...
                    map.push(mapMain)
                    if (isMobile()) {
                        document.getElementById("deviceController").style = "";
                        ...
                        document.getElementById("joy2Div").innerHTML = "";
                ...
                if (b[1] == "pos") {
                    if (mapMain.multiplayer != null && mapMain.multiplayer) {
                        ...
                        var x = Number(b[2]) / START_SIZE * SIZE;
                        var y = Number(b[3]) / START_SIZE * SIZE;
                        //console.log(x + " " + y)
                        // indexMoving++;
                        //   if(indexMoving==2){
                        // indexMoving=0;

                        // if(player.position.x != x && player.position.y != y || Math.abs(player.position.x-x)<1 && Math.abs(player.position.y-y)<1 ){
            ...
            if (b[0] == "lobby") {
                ...
                if (b[1] == "login") {
                    ...
                    player.id = getPlayerIs().id;
                    player.info = playerPreviewController.hash.get(player.id).json.levels[player.getPlayerIs()
                        .type]
                ...
                if (b[1] == "set_player") {
                    ...
                    player.id = player.getPlayerIs().id;
                    player.info = playerPreviewController.hash.get(player.id).json.levels[player.getPlayerIs()
                        .type]
                    player.info = playerPreviewController.hash.get(player.id).json.levels[player.getPlayerIs()
                        .type]
                ...
                if (b[1] == "restart") {
                    ...
                    var str = e.data.replace("game;captcha;", "")
                    document.getElementById("captcha").style = "display:block";
                    document.getElementById("auth").style = "display:none";
                    document.getElementById("reg").style = "display:none";
                    var code = '<img src="' + str + '">';
                    ...
                    alert("<?php echo $locale["CAPTCHA_NOT_CORRECT"];?>");
    ...
    async function run() {
        ...
        var jsonforimageloader = await loadJSON("res/index.json?id=" + Math.random());
    ...
    function reg() {
        ...
        document.getElementById("regpass").readOnly = true;

        var a = document.getElementById("reguser").value;
        var b = document.getElementById("regpass").value;

        if (a != null && b != null) {
            connection.send("reg;" + a + ";" + b);
        }
...
">
       ...
       margin-left: 30px;
       ">
                <div onclick="player.downKeyLeft();" class="fireButton" style="
    width: 50px;
    height: 50px;
"><img src="img/controller/left.png" style="width: 62%;pointer-events: none;"></div>
                ...
                <div class="fireButton" id="styleInt" style="
    width: 65px;
    height: 65px;
    font-weight: 600;
    font-size: 160%;
">6</div>
                <div class="fireButton" onclick="player.downKeyRight();" style="
    width: 50px;
    height: 50px;
"><img src="img/controller/right.png" style="width: 62%;pointer-events: none;"></div>
...
"><?php echo $locale["BUTTON_PAUSE"];?></div>
        ...
        <div id="joy1Div" style="display:none;width:160px;height:160px;position:absolute;bottom:6%;left:5%;zoom: 140%;">
        </div>
        <div id="joy2Div"
            style="display:none;  width:160px;height:160px;position:absolute;bottom:6%;right:5%;zoom: 140%;"></div>

        <div id="fps"></div>
        <div id="ping"></div>
        <div id="deviceController" style="display: none;">
            <div style="
      display: flex;
    position: absolute;
    bottom: 10%;
    ...
    zoom:150%;
        ">
                <div ontouchmove=' connection.send("battle;down;left");'
                    ontouchcancel='connection.send("battle;up;left");'
                    onmousedown=' connection.send("battle;down;left");'
                    ontouchstart=' connection.send("battle;down;left");'
                    ontouchend=' connection.send("battle;up;left");' onmouseup=' connection.send("battle;up;left");'
                    class="fireButton"><img src="img/controller/left.png" style="width: 62%;pointer-events: none;">
                </div>
                <div class="fireButton" ontouchmove=' connection.send("battle;down;right");'
                    ontouchcancel='connection.send("battle;up;right");'
                    ontouchstart=' connection.send("battle;down;right");'
                    ontouchend=' connection.send("battle;up;right");'><img src="img/controller/right.png"
                        style="pointer-events: none;width: 62%;"></div>
            </div>
    ...
    min-height: 100px;
        ...
        ">
                <div ontouchstart="on==false?connection.send('battle;down;shift'):connection.send('battle;up;shift'); console.log(this.children.superpower.src); on==false?this.children.superpower.src='img/controller/doublepower.png':this.children.superpower.src='img/controller/superpower.png'; on =!on;"
                    class="fireButton"><img id="superpower" src="img/controller/superpower.png"
                        style="pointer-events: none;width: 62%;"></div>




                <div class="fireButton" ontouchmove=" connection.send('battle;down;fire');"
                    ontouchcancel="connection.send('battle;up;fire');"
                    ontouchstart=" connection.send('battle;down;fire');"
                    ontouchend=" connection.send('battle;up;fire');"><img src="img/controller/fire.png"
                        style="pointer-events: none;width: 62%;"></div>
       ...
       min-height: 100px;
       ">
                <div ontouchstart="player.downKeyLeft();" class="fireButton" style="
    width: 50px;
    height: 50px;
"><img src="img/controller/left.png" style="width: 62%;pointer-events: none;"></div>
    ...
    font-size: 160%;
">2</div>
                <div class="fireButton" ontouchstart="player.downKeyRight();" style="
    width: 50px;
    height: 50px;
"><img src="img/controller/right.png" style="width: 62%;pointer-events: none;"></div>
       ...
       ">
                ...
                <div class="fireButton" ontouchmove=" connection.send('battle;down;up');"
                    ontouchstart=" connection.send('battle;down;up');"><img src="img/controller/jump.png"
                        style="pointer-events: none;width: 62%;"></div>
            </div>
        </div>
        ...
        <div id="heartPanel" style="display: none;"></div>
    ...
    <div id="base" style="display:none;">
        ...
        <a id="base_level" target="_blank" href="about.php?locale=<?php echo $lang;?>" style="bottom: calc(65px + 0%);"
            class="dws-submit"> <?php echo $locale["BUTTON_HELP"];?>
        </a>
    ...
    <div id="auth">
        <div class="container">
            ...
            <a href="#" onClick="upd();connection.send('reg;init');"><?php echo $locale["TEXT_REG"];?></a>
    ...
    <div id="reg" style="display:none;">
        <div class="container">
            ...
            <button class="dws-submit" id="buttonReg"
                onClick="reg(); return false;"><?php echo $locale["TEXT_REG"];?></button><br />
            <a href="#" onClick="window.location.reload();"><?php echo $locale["TEXT_LOGIN"];?></a>
    ...
    <div id="captcha" style="display:none;">
        <div class="container">
            ...
            <div class="dws-input">
                ...
                <input type="text" id="cap" name="username" placeholder="Captcha">
                <br>
                <div id="imgcap"></div>
            </div>
    ...
    <div class="container" id="errorWindow" style="display:none;">
        <div id="mainErrorBlock" style="color: white;
    margin: auto auto;
    font-size: 1em;
    width: 60%;
    top: 45%;
    ...
    width: 60%;
    height: 5%;
    margin-top: 0%;">CONNECTION FAIL</center><br />
            <center><a class="dws-submit" class="    margin-top: 10px;" onclick="window.location.reload();">RESTART</a>
            </center>
        </div>
